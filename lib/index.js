...
const { CachedChildCompilation } = require('./cached-child-compiler.js');

const {
  createHtmlTagObject,
  htmlTagObjectToString,
  HtmlTagArray,
} = require('./html-tags.js');
const prettyError = require('./errors.js');
const chunkSorter = require('./chunksorter.js');
const getHtmlRspackPluginHooks = require('./hooks.js').getHtmlRspackPluginHooks;
...
/** @typedef {import("../typings.js").TemplateParameter} TemplateParameter */
/** @typedef {import("@rspack/core").Compiler} Compiler */
/** @typedef {ReturnType<Compiler["getInfrastructureLogger"]>} Logger */
/** @typedef {import("@rspack/core").Compilation} Compilation */
/** @typedef {Array<{ name: string, source: import('@rspack/core').sources.Source, info?: import('@rspack/core').AssetInfo }>} PreviousEmittedAssets */
/** @typedef {{ publicPath: string, js: Array<string>, css: Array<string>, favicon?: string }} AssetsInformationByGroups */

class HtmlRspackPlugin {
  /**
   * @param {HtmlWebpackOptions} [userOptions]
   */
  constructor(userOptions = {}) {
    ...
    const defaultOptions = {
      ...
      publicPath:
        userOptions.publicPath === undefined ? 'auto' : userOptions.publicPath,
      hash: false,
  ...
  getPublicPath(compilation, filename, customPublicPath) {
    ...
    const webpackPublicPath = compilation.getAssetPath(
      /** @type {string} */ (compilation.outputOptions.publicPath),
      { hash: /** @type {string} */ (compilation.hash) },
    );
    // Webpack 5 introduced "auto" as default value
    const isPublicPathDefined = webpackPublicPath !== 'auto';

    const outputPath = /** @type {string} */ (compilation.options.output.path);
    let publicPath =
  ...
  postProcessHtml(
    ...
    const needMinify = typeof minify === 'function' && isProductionLikeMode;
  ...
  generateFavicon(
    ...
    const inputFileSystem =
      /** @type {NonNullable<Compilation['inputFileSystem']>} */ (
        compilation.inputFileSystem
      );

    return promisify(inputFileSystem.readFile)(filename)
      .then((buf) => {
  ...
  replacePlaceholdersInFilename(compiler, filename, fileContent, compilation) {
    ...
    const hash = compiler.webpack.util.createHash(
      /** @type {Parameters<typeof compiler.webpack.util.createHash>[0]} */ (
    ...
    if (compilation.outputOptions.hashSalt) {
      hash.update(compilation.outputOptions.hashSalt, 'utf8');
    }

    const contentHash = hash
      .digest(/** @type {string} */ (compilation.outputOptions.hashDigest))
      .slice(
        0,
        /** @type {number} */ (compilation.outputOptions.hashDigestLength),
      );
   ...
   * @param {Compilation} compilation
   * @param {string} outputName
   * @param {InstanceType<typeof CachedChildCompilation> | undefined} childCompilerPlugin
   * @param {PreviousEmittedAssets} previousEmittedAssets
  ...
  generateHTML(
    ...
    const templateResult = this.options.templateContent
      ...
      : /** @type {InstanceType<typeof CachedChildCompilation>} */ (
          childCompilerPlugin
        ).getCompilationEntryResult(this.options.template);
 ...
 *
 * Usage: HtmlRspackPlugin.getCompilationHooks(compilation).HOOK_NAME.tapAsync('YourPluginName', () => { ... });
 */
