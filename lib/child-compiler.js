 ...
 * for multiple HtmlRspackPlugin instances to improve the compilation performance.
 */
class HtmlWebpackChildCompiler {
  ...
  compileTemplates (mainCompilation) {
    ...
    const temporaryTemplateNames = this.templates.map((template, index) => `__child-HtmlRspackPlugin_${index}-${template}`);

    // Add all templates
    this.templates.forEach((template, index) => {
      new EntryPlugin(childCompiler.context, 'data:text/javascript,__webpack_public_path__ = __webpack_base_uri__ = htmlWebpackPluginPublicPath;', `HtmlRspackPlugin_${index}-${template}`).apply(childCompiler);
      new EntryPlugin(childCompiler.context, template, `HtmlRspackPlugin_${index}-${template}`).apply(childCompiler);
    });
    ...
    this.compilationPromise = new Promise((resolve, reject) => {
      ...
      childCompiler.hooks.thisCompilation.tap('HtmlRspackPlugin', (compilation) => {
